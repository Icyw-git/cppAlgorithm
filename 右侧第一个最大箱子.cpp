...
void getmax(int a[], int n) //单调栈
{
    ...
    int c[n];
    for (int i = n - 1; i >= 0; i--)
    //从右向左遍历数组，维护一个单调递减的栈，当前元素的可能的右侧第一个最大值就是栈顶元素，如果当前元素大于等于栈顶元素，则弹出栈顶元素，直到找到一个比当前元素大的元素或者栈为空，因为这些元素是不可能成为当前元素以及后续元素的右侧第一个最大值的，所以需要弹出，最后如果栈不为空，则当前元素的右侧第一个最大值就是栈顶元素，否则当前元素没有右侧第一个最大值。每次遍历完当前元素后，将当前元素入栈，继续遍历下一个元素。
    {
