...
void Clink::mergelinks(Clink& c1, Clink& c2)
//合并两个有序链表,思路:使用双指针,分别指向两个链表的头节点,比较两个指针指向的节点的数据,将较小的节点连接到合并后的链表的最后一个节点,然后将指向较小节点的指针指向下一个节点,重复此过程,直到其中一个链表为空,将另一个链表的剩余节点连接到合并后的链表的最后一个节点
{
    Node* p = c1.head->next;
    Node* q = c2.head->next;
