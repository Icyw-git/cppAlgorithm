...
void Clink::reverse() //链表逆置：利用三个指针prev、current和next，prev指向当前节点的前一个节点，current指向当前节点，next指向当前节点的下一个节点，在遍历链表的过程中，将current节点的指针域指向prev节点，然后将prev、current和next依次向后移动，直到current节点为nullptr，此时prev节点就是链表的最后一个节点，将头节点的指针域指向prev节点即可实现链表的逆置
{
